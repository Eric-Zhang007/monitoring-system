...
need_cmds=(python3 curl screen awk sed grep)
...
task_id="$(TASK_JSON="$task_submit" python3 - <<'PY'
import json
import os
...
while true; do
  ...
  read -r parsed_status parsed_error <<<"$(TASK_JSON="$task_payload" python3 - <<'PY'
import json
import os
...
if [[ "$RUN_HEALTH_CHECK_SCRIPT" == "1" ]]; then
  CHECK_DOCKER_SERVICES=0 API_BASE="$API_BASE" DATABASE_URL="${DATABASE_URL:-}" REDIS_URL="${REDIS_URL:-}" \
    python3 monitoring/health_check.py >/tmp/health_check_nodocker.log 2>&1 || {
      echo "[FAIL] monitoring/health_check.py returned non-zero"
      tail -n 80 /tmp/health_check_nodocker.log || true
      exit 2
    }
fi

echo "[OK] nodocker readiness passed"
